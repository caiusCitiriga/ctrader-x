...
describe('ts-proto codegen round-trip', () => {
    it('preserves required fields and the proto2 default of an omitted optional field', () => {
        ...
        const decoded = ProtoOAApplicationAuthReq.decode(ProtoOAApplicationAuthReq.encode(message).finish());

        expect(decoded).toEqual(message);
        expect(decoded.payloadType).toBe(ProtoOAPayloadType.PROTO_OA_APPLICATION_AUTH_REQ);
    });
    ...
    it('round-trips nested repeated messages', () => {
        ...
        const decoded = ProtoOASpotEvent.decode(ProtoOASpotEvent.encode(message).finish());
    ...
    it('round-trips an explicit enum value that differs from its proto2 default', () => {
        const message = ProtoOATrendbar.fromPartial({ volume: 1, period: ProtoOATrendbarPeriod.H1 });

        const decoded = ProtoOATrendbar.decode(ProtoOATrendbar.encode(message).finish());

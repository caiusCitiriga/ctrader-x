...
import { ProtoMessage, ProtoOAAccountAuthRes, ProtoOAApplicationAuthRes, ProtoOAErrorRes, ProtoOAOrderErrorEvent, ProtoOAPayloadType } from '../../src/types';
...
function wireAutoAuthServer(socket: net.Socket, onOtherRequest?: (request: ProtoMessage) => ProtoMessage | undefined): void {
    socket.resume();
    ...
    socket.on('data', (chunk: Buffer) => {
        for (const frame of decoder.push(chunk)) {
            ...
            if (request.payloadType === ProtoOAPayloadType.PROTO_OA_APPLICATION_AUTH_REQ) {
            ...
            } else if (request.payloadType === ProtoOAPayloadType.PROTO_OA_ACCOUNT_AUTH_REQ) {
                response = ProtoMessage.fromPartial({
            ...
            if (response) {
                socket.write(encodeFrame(ProtoMessage.encode(response).finish()));
            }
        }
    });
}
...
function createTestClient(
    ...
    options: { token?: ISpotwareOAuthToken; onOtherRequest?: (request: ProtoMessage) => ProtoMessage | undefined } = {}
): ITestClientHarness {
    ...
    const client = new SpotwareClient({
        ...
        token: options.token ?? { accessToken: 'access-token', refreshToken: 'refresh-token', tokenType: 'bearer', expiresIn: 2_628_000 },
...
describe('SpotwareClient', () => {
    ...
    it('authenticates on connect and forwards unsolicited messages', async () => {
        const { client, nextServerSocket } = createTestClient(server, port, oauthClient, createdClients);
        ...
        const unsolicited = ProtoMessage.fromPartial({ payloadType: 555, payload: new Uint8Array([9]) });
        serverSocket.write(encodeFrame(ProtoMessage.encode(unsolicited).finish()));

        await vi.waitFor(() => expect(messages.some((message) => message.payloadType === 555)).toBe(true));
    });
    ...
    it('correlates concurrent send() calls by clientMsgId even when responses arrive out of order', async () => {
        ...
        const first = client.send(GENERIC_REQUEST_PAYLOAD_TYPE, new Uint8Array([1]));
        const second = client.send(GENERIC_REQUEST_PAYLOAD_TYPE, new Uint8Array([2]));

        await vi.waitFor(() => expect(requests).toHaveLength(2));

        const respondTo = (clientMsgId: string | undefined, payload: Uint8Array) =>
            serverSocket.write(
                encodeFrame(
                    ProtoMessage.encode(
                        ProtoMessage.fromPartial({ payloadType: GENERIC_RESPONSE_PAYLOAD_TYPE, payload, clientMsgId })
                    ).finish()
                )
            );
    ...
    it('rejects send() with a typed error on a correlated PROTO_OA_ERROR_RES', async () => {
        const { client } = createTestClient(server, port, oauthClient, createdClients, {
            onOtherRequest: (request) =>
                ProtoMessage.fromPartial({
                    payloadType: ProtoOAPayloadType.PROTO_OA_ERROR_RES,
                    payload: ProtoOAErrorRes.encode(ProtoOAErrorRes.fromPartial({ errorCode: 'SYMBOL_NOT_FOUND' })).finish(),
        ...
        await expect(client.send(GENERIC_REQUEST_PAYLOAD_TYPE, new Uint8Array())).rejects.toMatchObject({ errorCode: 'SYMBOL_NOT_FOUND' });
    });
    ...
    it('rejects send() with a typed error on a correlated PROTO_OA_ORDER_ERROR_EVENT', async () => {
            ...
            onOtherRequest: (request) =>
                ProtoMessage.fromPartial({
                    ...
                    payload: ProtoOAOrderErrorEvent.encode(
                        ProtoOAOrderErrorEvent.fromPartial({ ctidTraderAccountId: TEST_ACCOUNT_ID, errorCode: 'NOT_ENOUGH_MONEY' })
                    ).finish(),
                    clientMsgId: request.clientMsgId
                })
        });
        ...
        await expect(client.send(GENERIC_REQUEST_PAYLOAD_TYPE, new Uint8Array())).rejects.toMatchObject({ errorCode: 'NOT_ENOUGH_MONEY' });
    });
    ...
    it('times out a send() call when no response ever arrives', async () => {
        ...
        await expect(client.send(GENERIC_REQUEST_PAYLOAD_TYPE, new Uint8Array())).rejects.toBeInstanceOf(SpotwareRequestError);
    });
    ...
    it('rejects in-flight send() calls immediately on disconnect, without waiting out their own timeout', async () => {
        ...
        (client as unknown as { requestTimeoutMs: number }).requestTimeoutMs = 5_000;

        await client.connect();

        const pending = client.send(GENERIC_REQUEST_PAYLOAD_TYPE, new Uint8Array());
    ...
    it('refreshes an expiring token once for concurrent send() calls, not once per call', async () => {
        const refreshSpy = vi.spyOn(oauthClient, 'refreshAccessToken').mockResolvedValue({

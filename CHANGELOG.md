...
- **`dotenv` moved from `dependencies` to `devDependencies`.** It's only ever imported by the runnable examples under `src/example/`, never by the SDK itself — every consumer of the package was installing it for no reason.
- **The published tarball no longer includes `dist/example/`.** `files` now excludes it (`"!dist/example"`) while leaving the local build untouched, so `npm run start:prod` still works from a checkout; it just isn't shipped to consumers. Package size dropped from 195 to 159 files (197 kB → 182 kB packed).
- **Added `"sideEffects": false`** to `package.json`, so bundlers (Webpack, Rollup, esbuild) can tree-shake unused exports. Verified no module under `src/` (outside the examples) runs code at import time.

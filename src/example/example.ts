...
async function main(): Promise<void> {
    ...
    console.log(
        '\nSending PROTO_OA_VERSION_REQ through SpotwareClient to confirm request/response correlation works end to end...',
    );
    const response = await client.send(
        ProtoOAPayloadType.PROTO_OA_VERSION_REQ,
        ProtoOAVersionReq.encode(ProtoOAVersionReq.fromPartial({})).finish(),
    );
    const version = ProtoOAVersionRes.decode(
        response.payload ?? new Uint8Array(),
    );
    console.log(`Server version: ${version.version}`);

    // This intentionally runs indefinitely so that the user can test the reconnection
    // mechanism, by switching the wifi on/off, and wait for the reconnection logs
    // to appear before turning it back on.
    setInterval(async () => {
        console.log('*'.repeat(50));
        try {
            const response = await client.send(
                ProtoOAPayloadType.PROTO_OA_VERSION_REQ,
                ProtoOAVersionReq.encode(
                    ProtoOAVersionReq.fromPartial({}),
                ).finish(),
            );
            const version = ProtoOAVersionRes.decode(
                response.payload ?? new Uint8Array(),

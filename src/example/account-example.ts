...
async function main(): Promise<void> {
    ...
    const netUnrealized = unrealized.reduce((total, position) => total + position.netUnrealizedPnL, 0);
    console.log(`\nOpen positions: ${unrealized.length}, net unrealized PnL: ${netUnrealized}`);
    console.log(`Equity (balance + net unrealized): ${balance + netUnrealized}`);
    ...
    account.on('traderUpdated', (updated) => console.log('traderUpdated: balance is now', updated.balance));
    account.on('marginChanged', (change) => console.log('marginChanged:', change));
    account.on('marginCallTriggered', (marginCall) => console.log('MARGIN CALL:', marginCall));
    account.on('tokenInvalidated', (accountIds, reason) => console.log('token revoked for', accountIds, reason));

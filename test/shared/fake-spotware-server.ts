...
export function wireFakeServer(
    ...
    socket.on('data', (chunk: Buffer) => {
        for (const frame of decoder.push(chunk)) {
            ...
            const response = options.onOtherRequest?.(request) ?? buildResponse(request, symbols);
            if (response) {
...
function buildResponse(request: ProtoMessage, symbols: ProtoOALightSymbol[]): ProtoMessage | undefined {

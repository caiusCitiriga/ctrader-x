...
async function main(): Promise<void> {
    ...
    console.log(`Got ${ticks.ticks.length} ticks${ticks.hasMore ? ' (more available — narrow the range)' : ''}. Newest five:`);
    console.table(
    ...
    marketData.on('trendbar', (symbolId, bar) => {
        ...
        console.log(`[bar ${symbolId}] ${at} O=${bar.open} H=${bar.high} L=${bar.low} C=${bar.close} V=${bar.volume}`);
    });

    let depthCount = 0;
    marketData.on('depth', (update) => {
        depthCount += 1;
        const best = update.newQuotes.slice(0, 2).map((quote) => (quote.bid ? `bid ${quote.bid}` : `ask ${quote.ask}`));
        console.log(`[depth ${update.symbolId}] +${update.newQuotes.length} -${update.deletedQuoteIds.length} ${best.join(' ')}`);
    });
    ...
    try {
        ...
        console.log('This broker refused depth for this symbol:', (error as Error).message);
    }
